    ...
    try {
        switch (event.httpMethod) {
            ...
            case 'POST': {
                ...
                const { userId, contractType, startDate, endDate, post, department, unit, grossSalary, netSalary, currency, contractFile } = payload || {};
                // Basic validation
                const required = {
                    userId,
                    contractType,
            ...
            case 'PUT': {
                ...
                if (payload.post)
                    updateData.post = payload.post;
                if (payload.department)
                    updateData.department = payload.department;
                if (payload.unit !== undefined)
                    updateData.unit = payload.unit || null;
                if (payload.grossSalary !== undefined)
                    updateData.grossSalary = Number(payload.grossSalary);
                if (payload.netSalary !== undefined)
                    updateData.netSalary = Number(payload.netSalary);
                if (payload.currency)
                    updateData.currency = payload.currency;
                if (payload.contractFile !== undefined)
                    updateData.contractFile = payload.contractFile || null;
